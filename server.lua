...
local function isAllowed(source, perm)
    if Config.UseESX then
        ...
        return hasAllowedJob or IsPlayerAceAllowed(source, perm)
    else
...
RegisterCommand('jail', function(src, args, raw)
    -- /jail <id> <time>
    if isAllowed(src, "Badger_Jailing.Jail") then
        if #args < 2 then
            -- Not enough args 
        ...
        if GetPlayerIdentifiers(args[1])[1] ~= nil then
            ...
            if tonumber(args[2]) ~= nil then
                ...
                TriggerClientEvent('chatMessage', -1, Config.Prefix .. "Player ^5" .. GetPlayerName(args[1]) ..
                    " ^3has been jailed for ^1" .. args[2] .. " ^3seconds...")
                Citizen.CreateThread(function()
                    ...
                    while not IsCellFree() do
                        TriggerClientEvent('chatMessage', args[1], Config.Prefix .. "Waiting on a free prison cell...")
                        Citizen.Wait(10000)
                    end
                ...
                TriggerClientEvent('chatMessage', src,
                    Config.Prefix .. "^1ERROR: The ^2time ^1argument was not a valid number.")
            end
        else
            -- Invalid player 
            TriggerClientEvent('chatMessage', src, Config.Prefix .. "^1ERROR: Invalid player ID.")
        end
    end
end)
...
RegisterCommand('free', function(src, args, raw)
    -- /unjail <id> 
    if isAllowed(src, "Badger_Jailing.Unjail") then
        if #args ~= 1 then
            -- Not enough args 
        ...
        if GetPlayerIdentifiers(args[1])[1] ~= nil then
            -- Valid player 
            TriggerClientEvent('chatMessage', -1, Config.Prefix .. "Player ^5" .. GetPlayerName(args[1]) ..
                " ^3has been released from jail by ^2" .. GetPlayerName(src))
...
AddEventHandler("Badger_Jailing:Connected", function()
    ...
    if cfg[ids.license] ~= nil then
        ...
        if CellTracker[cell] == nil then
            ...
            Citizen.CreateThread(function()
                while not IsCellFree() do
                    TriggerClientEvent('chatMessage', src, Config.Prefix .. "Waiting on a free prison cell...")
                    Citizen.Wait(10000)
                end
                local key = GetFreeCell()
                local coords = Config.Cells[key]
